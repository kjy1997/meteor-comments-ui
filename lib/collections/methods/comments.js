...
Meteor.methods({
  'comments/add'(referenceId, content, anonData) {
    const to = Meteor.users.findOne({ _id: referenceId });
    const from = Meteor.users.findOne({ _id: this.userId });
    let emailOptions = {
      from: 'guochengwei170@gmail.com',
      to: to.emails[0].address,
      subject: 'Froomie Notification',
      html: `<p>Hi ${to.username}, ${from.username} just commented your profile. Check out <a href="http://localhost:3000/profilemain">here</a><p>`
    }
